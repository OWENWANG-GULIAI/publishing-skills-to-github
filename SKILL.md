...
2. 先形成事实清单：用途、用户、触发方式、输入、输出、工作流、依赖、边界、版本、许可证、仓库 owner/name，以及经过验证的安装方式；有品牌要求时还要核实获授权的品牌资产与使用范围。
...
7. 用户明确要求 GULIAI、谷燕燕或指定 GULIAI 品牌方案，并提供或确认可用 Logo 时，进入 **GULIAI 品牌发布模式**：README 首屏按 Logo → 标题 → 中文价值主张 → 真实版本/语言/许可证徽章的顺序呈现，并在其后保留“定位 / 不做什么”卡片。没有此授权时使用中性 Hero，不得擅自植入品牌或重绘 Logo。
8. 只暂存本次目标文件，不覆盖脏工作区中的无关用户改动。

## 执行约定

按照发布流程中的“解析目标 → 完整检查 → 事实清单 → 选择中性或 GULIAI 品牌 README → 审计 → 差异复核 → 发布 → 远端回读”执行。提交或推送前运行：
...
- 本地 README 达到专业结构要求，并准确反映当前 Skill；在 GULIAI 品牌发布模式下，Logo、定位卡和三类真实徽章也已按模板核验；
- 确定性审计和相关测试通过；
