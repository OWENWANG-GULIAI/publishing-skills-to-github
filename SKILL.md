...
2. 先形成事实清单：用途、用户、触发方式、输入、输出、工作流、依赖、边界、版本、许可证、仓库 owner/name，以及经过验证的安装方式。
...
7. 只暂存本次目标文件，不覆盖脏工作区中的无关用户改动。

## 执行约定

按照发布流程中的“解析目标 → 完整检查 → 事实清单 → README → 审计 → 差异复核 → 发布 → 远端回读”执行。提交或推送前运行：
...
- 本地 README 达到专业结构要求，并准确反映当前 Skill；
- 确定性审计和相关测试通过；
